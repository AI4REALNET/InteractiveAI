...
from .ia_flatland.src.flatland_ai.recommender import Recommender


class SNCFManager(BaseRecommendationManager):

    def __init__(self) -> None:
        super().__init__()
        script_dir = os.path.dirname(os.path.abspath(__file__))
        self.owl_file_path = os.path.join(
            script_dir, "ontology/final_populate_v1.owl"
        )
        self.graph_module_path = os.path.join(
            script_dir, "ia_flatland/models/graph_module_real.pkl"
        )
        self.ai_model_path = os.path.join(
            script_dir, "ia_flatland/models/ppo_flatland_cab_real.pth"
        )
        self.recommender = Recommender(
            self.graph_module_path, self.ai_model_path
        )
    ...
    def get_recommendation(self, request_data):
        ...
        if event_data["event_type"] == "PASSENGER":
            ...
            fake_transport_plan_1 = (
                self.recommender.get_recommended_transport_plan(
...
TGV 8420 : Passer par la ligne classique entre Angoulême et Poitiers.",
        ...
        fake_targets_ids_2 = self.recommender.run_idle(
            context_data, event_data
        )
        fake_transport_plan_2 = (
            self.recommender.get_recommended_transport_plan(
        ...
        recommendations = self.recommender.recommend(
            context_data, event_data, models=["AI", "Heuristic"]
        )
